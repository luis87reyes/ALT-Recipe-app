...
class InputsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create input" do
    assert_difference('Input.count') do
      post inputs_url, params: { input: { directions: @input.directions, name: @input.name } }
    end
  ...
  test "should update input" do
    patch input_url(@input), params: { input: { directions: @input.directions, name: @input.name } }
